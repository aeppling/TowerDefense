...
void MissileLauncher::shoot(int towerPosX, int towerPosY, TDUnit *target, float missileSpeed) {
    ...
    this->_missiles.at(this->_missiles.size() - 1)->startThread(this->_sfmlMissileLoader, {(float)towerPosX, (float)towerPosY},target->getSprite().getPosition(), this->_cellSize, missileSpeed, this->_typeName);
}
